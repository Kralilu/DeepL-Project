...
class Args(Tap):
    ...
    def seed_everything(self, benchmark: bool):
        ...
        if self.seed is None:
            ...
            # seed = self.seed * dist.get_world_size() + dist.get_rank() -> allocates different seeds for each rank
            seed = self.seed
    ...
    def dump_log(self):
        ...
        if '1/' in self.cur_ep: # first time to dump log
            with open(self.log_txt_path, 'w') as fp:
                json.dump({'is_master': True, 'name': self.exp_name, 'cmd': self.cmd, 'commit': self.commit_id, 'branch': self.branch, 'tb_log_dir_path': self.tb_log_dir_path}, fp, indent=0)
                fp.write('\n')
...
def init_dist_and_get_args():
    ...
    if not hasattr(args, 'local_debug'):
